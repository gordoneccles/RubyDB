...
class BelongsToOptions < AssocOptions
  ...
  def initialize(owner, options = {})
    ...
    @table_name = options[:table_name] || owner.to_s.downcase.pluralize.underscore
...
class HasManyOptions < AssocOptions
  def initialize(ownees, my_class_name, options = {})
    ...
    @table_name = options[:table_name] || ownees.to_s.pluralize.underscore
...
module Associatable
  ...
  def has_many(ownees, options = {})
    ownee_db_attributes = HasManyOptions.new(ownees, self.to_s, options)

    assoc_options[ownees] = ownee_db_attributes
    ...
    define_method(ownees.to_s) do
      results = DBConnection.execute(<<-SQL)
        SELECT
          *
        FROM
          #{ownee_db_attributes.table_name}
        WHERE
          #{ownee_db_attributes.table_name}.#{ownee_db_attributes.foreign_key} = #{self.send(:id)}
      SQL

      ownee_db_attributes.model_class.parse_all(results)
    end
  end
  ...
  def has_many_through(target_name, relay_name, relays_name_for_target)
    relay_db_attributes = self.assoc_options[relay_name]
    ...
    define_method(target_name) do
      ...
      results = DBConnection.execute(<<-SQL, id)
        ...
        JOIN
          #{relay_db_attributes.table_name} ON
            #{relay_db_attributes.table_name}.#{relay_db_attributes.foreign_key} = #{self.class.table_name}.id
        JOIN
          #{target_db_attributes.table_name} ON
            #{target_db_attributes.table_name}.#{target_db_attributes.foreign_key} = #{relay_db_attributes.table_name}.id
        WHERE
          #{self.class.table_name}.id = ?
      SQL

      target_db_attributes.model_class.parse_all(results)
